    ...
    fn synthesize(
        ...
        layouter.assign_table(
            ...
            |mut table| {
                ...
                let mut transitions: Vec<(F, F, F)> = vec![(F::ZERO, F::ZERO, F::ZERO)];
                ...
                // assign the table
                for (offset, (st_cur, st_nxt, char)) in transitions.into_iter().enumerate() {
                    table.assign_cell(
                        || format!("key"),
                        config.tbl_st_cur,
                        offset,
                ...
                for i in 0..MAX_STR_LEN {
                    ...
                    region.assign_advice(
                        ...
                        || {
                            self.sts
                                .as_ref()
                                .map(|s| F::from(s.get(i).cloned().unwrap_or(ST_DONE) as u64))
                        },
                    )?;
