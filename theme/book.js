...
(function themes() {
  ...
  function updateThemedImg(theme) {
    // Update any themed images
    document.querySelectorAll('img[src*="-themed"]').forEach((img) => {
      if (img.src.match(/-(dark|light)-themed\.[a-z]+$/i)) {
        if (theme.match(/coal|navy|ayu/)) {
          img.src = img.src.replace(/-light-themed\./, "-dark-themed.");
        } else {
          img.src = img.src.replace(/-dark-themed\./, "-light-themed.");
        }
      }
    });
  }
